...
its error is rounding: the largest relative monomial error over all monomials of degree `≤ p` is
below `5.2e-15` for every GH rule (most are `1e-16`–`1e-15`; the worst is `d = 5`, `p = 21`) and
below `4.5e-16` for every Le rule. All weights are positive, and all nodes of the Le rules lie
strictly inside the cube. The gate applied when the data were built, and again by the test suite
on every rule, is `1e-11`. The measured value of each rule is in the catalog (`relerr`) and in
[`RULES.md`](RULES.md).
